...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  if (typeof body.student_name === "string") allowed.student_name = body.student_name.trim().slice(0, 200);
  ...
  const { data, error } = await supabase
    .from("revenues").update(allowed).eq("id", id).select().single();
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({ success: true, message: "Revenu annulé." });
}
